...
func AssertValidCharacter(t *testing.T, char *models.Character) {
	...
	assert.Greater(t, char.MaxHP, 0, "Character MaxHP should be greater than 0")
	
	// Validate stats
	AssertValidStats(t, &char.Stats)
}

// AssertValidStats validates character stats
func AssertValidStats(t *testing.T, stats *models.CharacterStats) {
	t.Helper()
	
	assert.GreaterOrEqual(t, stats.Strength, 1, "Strength should be at least 1")
	assert.LessOrEqual(t, stats.Strength, 30, "Strength should not exceed 30")
	assert.GreaterOrEqual(t, stats.Dexterity, 1, "Dexterity should be at least 1")
	assert.LessOrEqual(t, stats.Dexterity, 30, "Dexterity should not exceed 30")
	assert.GreaterOrEqual(t, stats.Constitution, 1, "Constitution should be at least 1")
	assert.LessOrEqual(t, stats.Constitution, 30, "Constitution should not exceed 30")
	assert.GreaterOrEqual(t, stats.Intelligence, 1, "Intelligence should be at least 1")
	assert.LessOrEqual(t, stats.Intelligence, 30, "Intelligence should not exceed 30")
	assert.GreaterOrEqual(t, stats.Wisdom, 1, "Wisdom should be at least 1")
	assert.LessOrEqual(t, stats.Wisdom, 30, "Wisdom should not exceed 30")
	assert.GreaterOrEqual(t, stats.Charisma, 1, "Charisma should be at least 1")
	assert.LessOrEqual(t, stats.Charisma, 30, "Charisma should not exceed 30")
}
...
func AssertValidCombatant(t *testing.T, combatant *models.Combatant, index int) {
	...
	assert.Contains(t, []models.CombatantType{models.CombatantTypePlayer, models.CombatantTypeNPC, models.CombatantTypeMonster}, 
		combatant.Type, "Combatant[%d] Type should be valid", index)
	assert.GreaterOrEqual(t, combatant.HitPoints, 0, "Combatant[%d] HitPoints should not be negative", index)
	assert.Greater(t, combatant.MaxHP, 0, "Combatant[%d] MaxHP should be greater than 0", index)
	assert.LessOrEqual(t, combatant.HitPoints, combatant.MaxHP, 
		"Combatant[%d] HitPoints should not exceed MaxHP", index)
}
...
// AssertValidInventory validates an inventory entry
func AssertValidInventory(t *testing.T, inv *models.Inventory) {
	t.Helper()
	
	require.NotNil(t, inv, "Inventory should not be nil")
	assert.NotEmpty(t, inv.ID, "Inventory ID should not be empty")
	assert.NotEmpty(t, inv.CharacterID, "Inventory CharacterID should not be empty")
	assert.NotEmpty(t, inv.ItemID, "Inventory ItemID should not be empty")
	assert.Greater(t, inv.Quantity, 0, "Inventory Quantity should be greater than 0")
}
...
func AssertValidDiceRoll(t *testing.T, roll *models.DiceRoll) {
	...
	assert.NotEmpty(t, roll.Notation, "DiceRoll Notation should not be empty")
	...
	assert.NotEmpty(t, roll.Rolls, "DiceRoll should have at least one roll")
	
	// Validate individual rolls
	for i, r := range roll.Rolls {
		assert.Greater(t, r.Sides, 0, "Roll[%d] Sides should be greater than 0", i)
		assert.GreaterOrEqual(t, r.Result, 1, "Roll[%d] Result should be at least 1", i)
		assert.LessOrEqual(t, r.Result, r.Sides, "Roll[%d] Result should not exceed sides", i)
	}
}
